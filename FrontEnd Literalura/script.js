...
// Mostrar formulario para buscar un libro por título (filtrado en el front end)
...
// Función para buscar libro por título: se obtiene la lista completa de libros y se filtra localmente
...
function autoresVivosEnAnio(anio) {
  // Se usa "año" como nombre del parámetro; se codifica para evitar problemas con el carácter ñ.
  fetch(`${API_BASE_URL}/autores/vivos?${encodeURIComponent('año')}=${anio}`)
    .then(response => response.json())
    .then(data => displayResults(data))
    .catch(error => {
...
function showLibrosPorIdiomaForm() {
  ...
  formContainer.innerHTML = `
    <form id="librosPorIdiomaForm">
      <input type="text" id="idioma" placeholder="Ingrese Idioma (es, en, fr, pt)" required>
...
function librosPorIdioma(idioma) {
  fetch(`${API_BASE_URL}/libros/idioma?idioma=${encodeURIComponent(idioma)}`)
    .then(response => response.json())
    .then(data => displayResults(data))
    .catch(error => {
